// ESM port of electron/prompt-engine.js for the browser (web) build.
// Keep in sync with electron/prompt-engine.js — same SYSTEM_PROMPT, ACTIONS, and
// message-building logic so web and desktop produce identical prompts.

export const SYSTEM_PROMPT = `You are PromptForge, a coding-only prompt engineer.
 ...
 */
export function buildMessages({ input, promptType, targetAgent, clarify = false, projectContext = '' } = {}) {
  const lines = [];
 ...
 */
export function buildActionMessages({ currentPrompt, action } = {}) {
 ...
 */
export function buildProjectContext(project = {}, files = []) {
  const out = [];
